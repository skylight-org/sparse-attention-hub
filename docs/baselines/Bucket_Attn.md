...
**Bucket Attention** is a sparse attention mechanism inspired by  
**RACE (Repeated Arrays of Count Estimators)** and **LSH-based soft hashing**.
...
5. Performs **value-aware ranking** to produce the final Top-K sparse mask.

This procedure is:

- **Deterministic** (no sampling)
- **Fully vectorized** (no loops per table/query)

Bucket Attention leverages geometric hashing to approximate attention neighborhoods efficiently.
...
2. Apply nonlinearity:  
   ...
   \tanh(z_{\ell,p}(q))
   \]
3. Compute similarity to all **R hypercube corners** \( c_r \in \{-1,+1\}^P \):
...
2. Select the **Top-K** keys by `score(q,i)` for each query.
3. Produce the final sparse attention mask:
