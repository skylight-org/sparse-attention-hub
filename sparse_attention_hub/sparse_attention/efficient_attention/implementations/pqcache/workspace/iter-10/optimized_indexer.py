"""Iteration 10: Use torch.compile to fuse scattered operations.

This automatically reduces kernel launches from 29 → ~5-10 by fusing:
- Multiple torch.arange calls
- Tensor copy operations  
- Index assignment operations

Expected improvement: ~100-150 μs (30-35% gain over iter-9)
"""
...
def __indexer_next_uncompiled(
    ...
    """Base implementation (same as iter-9, will be compiled)."""
...
# Compile for operation fusion
# Use "default" mode instead of "reduce-overhead" to avoid CUDA graphs issues
# with tensor mutations (line 182: torch.cat with cached_codebook)
__indexer_next = torch.compile(
    __indexer_next_uncompiled,
    mode="default",    # Standard optimization (no CUDA graphs)
    fullgraph=False,   # Allow Triton kernel passthrough
)

__all__ = ['__indexer_next']
