"""Optimized implementation of PQCache indexer_next logic - Iteration 2.

This module contains an optimized version with improved memory access patterns
using shared memory and better blocking strategies.
"""
...
@triton.autotune(
    configs=[
        triton.Config({"BLOCK_KEYS": 64, "BLOCK_QUERIES": 1}, num_warps=4),
        triton.Config({"BLOCK_KEYS": 128, "BLOCK_QUERIES": 1}, num_warps=4),
        triton.Config({"BLOCK_KEYS": 256, "BLOCK_QUERIES": 1}, num_warps=8),
        triton.Config({"BLOCK_KEYS": 64, "BLOCK_QUERIES": 2}, num_warps=4),
        triton.Config({"BLOCK_KEYS": 128, "BLOCK_QUERIES": 2}, num_warps=4),
    ],
    key=["n_clustered", "n_subvec", "subvec_d"],
)
@triton.jit
def pq_score_kernel_v2(
    # Input pointers
    ...
    """Optimized Triton kernel for computing PQ-based attention scores.
    
    Improvements over v1:
    - 2D blocking (queries × keys) for better parallelism
    ...
    # Compute 2D grid: (batch * head * query_blocks) × key_blocks
    num_query_blocks: tl.int32 = tl.cdiv(sq, BLOCK_QUERIES)
    num_key_blocks: tl.int32 = tl.cdiv(n_clustered, BLOCK_KEYS)
    
    # Decompose pid into (batch, head, query_block, key_block)
    total_query_blocks: tl.int32 = b * h * num_query_blocks
    query_block_id: tl.int32 = pid // num_key_blocks
    key_block_id: tl.int32 = pid % num_key_blocks
    
    # Extract batch, head, query_block from query_block_id
    batch_idx: tl.int32 = query_block_id // (h * num_query_blocks)
    rem: tl.int32 = query_block_id % (h * num_query_blocks)
    head_idx: tl.int32 = rem // num_query_blocks
    query_block_idx: tl.int32 = rem % num_query_blocks
    
    # Compute query and key ranges for this block
    query_start: tl.int32 = query_block_idx * BLOCK_QUERIES
    key_start: tl.int32 = key_block_id * BLOCK_KEYS
    
    query_offsets = query_start + tl.arange(0, BLOCK_QUERIES)
    key_offsets = key_start + tl.arange(0, BLOCK_KEYS)
    
    query_mask = query_offsets < sq
    key_mask = key_offsets < n_clustered
    
    # Initialize score accumulator [BLOCK_QUERIES, BLOCK_KEYS]
    scores_accum = tl.zeros([BLOCK_QUERIES, BLOCK_KEYS], dtype=tl.float32)
    
    # Loop over subvectors
    for subvec_idx in range(n_subvec):
        # Load codebook indices for this subvector and key block: [BLOCK_KEYS]
        ...
        # Compute scores for this subvector via dot product
        # Load queries for this subvector: [BLOCK_QUERIES, subvec_d]
        ...
        # Load centroids base pointer for this subvector
        ...
        # Compute Q @ C^T where C is the selected centroids
        # For each dimension, load query values and corresponding centroid values
        for d_idx in range(subvec_d):
            # Load query values for all queries in block: [BLOCK_QUERIES]
            q_ptrs = (
                q_base_ptr
                + query_offsets[:, None] * q_stride_sq
                + d_idx * q_stride_d
            )
            q_vals = tl.load(q_ptrs, mask=query_mask[:, None], other=0.0)
            
            # Load centroid values for all keys in block: [BLOCK_KEYS]
            # Each key has a codebook index pointing to a centroid
            c_ptrs = c_base_ptr + cb_indices * c_stride_cent + d_idx * c_stride_d
            c_vals = tl.load(c_ptrs, mask=key_mask, other=0.0)
            
            # Outer product and accumulate: [BLOCK_QUERIES, BLOCK_KEYS]
            scores_accum += q_vals * c_vals[None, :]
    
    # Store results: [BLOCK_QUERIES, BLOCK_KEYS]
    ...
    # Create 2D mask and pointers
    store_mask = query_mask[:, None] & key_mask[None, :]
    store_ptrs = (
        scores_base_ptr
        + query_offsets[:, None] * s_stride_sq
        + key_offsets[None, :] * s_stride_clustered
    )
    
    tl.store(store_ptrs, scores_accum, mask=store_mask)


def compute_pq_scores_triton_v2(
    queries: torch.Tensor,
    centroids: torch.Tensor,
    codebook: torch.Tensor,
) -> torch.Tensor:
    """Compute PQ-based attention scores using optimized Triton kernel v2.
    ...
    # Grid will be computed based on the selected block sizes
    def grid(meta):
        return (
            b * h * triton.cdiv(sq, meta["BLOCK_QUERIES"]) * triton.cdiv(n_clustered, meta["BLOCK_KEYS"]),
        )
    
    pq_score_kernel_v2[grid](
        queries,
        centroids,
        codebook,
        scores,
...
def __indexer_next(
    ...
    """Optimized indexer logic for PQCache pattern - Iteration 2.
    
    This version uses an improved Triton kernel with better memory access patterns.
    """
    # Get dimensions
    b: int
    h: int
    ...
    # Compute PQ-based scores using optimized Triton kernel v2
    ...
    # Use optimized Triton kernel v2 for score computation
    scores: torch.Tensor = compute_pq_scores_triton_v2(
        queries_reshaped,
        repeat_centroids,
        repeat_codebook,
    )
